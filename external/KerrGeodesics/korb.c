// This software describes the generic bound motion of a test mass around a Kerr
// black hole. Copyright (C) 2019  Thomas Osburn
//
...
double korb_dladpsi(double psi, korb_params orbpar) {
  return (1 - orbpar.e * orbpar.e) /
         sqrt(fabs((1 - orbpar.E * orbpar.E) *
                   (orbpar.p - orbpar.p3 -
                    orbpar.e * (orbpar.p + orbpar.p3 * cos(psi))) *
                   (orbpar.p - orbpar.p4 +
                    orbpar.e * (orbpar.p - orbpar.p4 * cos(psi)))));
}
...
double korb_dpsidla(double psi, korb_params orbpar) {
  return sqrt(fabs((1 - orbpar.E * orbpar.E) *
                   (orbpar.p - orbpar.p3 -
                    orbpar.e * (orbpar.p + orbpar.p3 * cos(psi))) *
                   (orbpar.p - orbpar.p4 +
                    orbpar.e * (orbpar.p - orbpar.p4 * cos(psi))))) /
...
/*!
 * Find T_r, which is the radial part of the derivative of t with respect to
 * Mino time (Eq. 2.19)
 */
double korb_Tr(double psi, korb_params orbpar) {
  double r = korb_rfrompsi(psi, orbpar);
  return orbpar.E * (r * r + orbpar.a * orbpar.a) *
             (r * r + orbpar.a * orbpar.a) / korb_D(r, orbpar) +
         orbpar.a * orbpar.Lz *
             (1 - (r * r + orbpar.a * orbpar.a) / korb_D(r, orbpar));
}

/*!
 * This function provides the integrand for calculating Mino time Fourier
 * coefficients of T_r
 */
double complex korb_Trint(double psi, int n, korb_params orbpar) {
  double la = korb_lafrompsi(psi, orbpar);
  return orbpar.Yr * korb_dladpsi(psi, orbpar) * korb_Tr(psi, orbpar) *
         (cos(n * orbpar.Yr * la) + I * sin(n * orbpar.Yr * la));
}

/*!
 * Find T_theta, which is the polar part of the derivative of t with respect to
 * Mino time (Eq. 2.20)
 */
...
/*!
 * This function provides the integrand for calculating Mino time Fourier
 * coefficients of T_theta
 */
double complex korb_Tthint(double chi, int n, korb_params orbpar) {
  double la = korb_lafromchi(chi, orbpar);
  return orbpar.Yth * korb_dladchi(chi, orbpar) * korb_Tth(chi, orbpar) *
         (cos(n * orbpar.Yth * la) + I * sin(n * orbpar.Yth * la));
}

/*!
 * Find Psi_r, which is the radial part of the derivative of phi with respect to
 * Mino time (Eq. 2.17)
 */
double korb_Pr(double psi, korb_params orbpar) {
  double r = korb_rfrompsi(psi, orbpar);
  return orbpar.a * orbpar.E *
             ((r * r + orbpar.a * orbpar.a) / korb_D(r, orbpar) - 1) -
         orbpar.a * orbpar.a * orbpar.Lz / korb_D(r, orbpar);
}

/*!
 * This function provides the integrand for calculating Mino time Fourier
 * coefficients of Psi_r
 */
double complex korb_Print(double psi, int n, korb_params orbpar) {
  double la = korb_lafrompsi(psi, orbpar);
  return orbpar.Yr * korb_dladpsi(psi, orbpar) * korb_Pr(psi, orbpar) *
         (cos(n * orbpar.Yr * la) + I * sin(n * orbpar.Yr * la));
}

/*!
 * Find Psi_theta, which is the polar part of the derivative of phi with respect
 * to Mino time (Eq. 2.18)
 */
...
/*!
 * This function provides the integrand for calculating Mino time Fourier
 * coefficients of Psi_theta
 */
double complex korb_Pthint(double chi, int n, korb_params orbpar) {
  double la = korb_lafromchi(chi, orbpar);
  return orbpar.Yth * korb_dladchi(chi, orbpar) * korb_Pth(chi, orbpar) *
         (cos(n * orbpar.Yth * la) + I * sin(n * orbpar.Yth * la));
}

/*!
 * This function provides the integrand for calculating Mino time Fourier
 * coefficients of the derivative of chi_theta with respect to Mino time
 */
...
/*!
 * This function provides the integrand for calculating Mino time Fourier
 * coefficients of the derivative of chi_r with respect to Mino time
 */
...
/*!
 * This function returns the integral of T_r with respect to Mino time given the
 * Fourier coefficients of T_r (Eq. 3.19)
 */
...
/*!
 * This function returns the integral of T_theta with respect to Mino time given
 * the Fourier coefficients of T_theta (Eq. 3.20)
 */
...
/*!
 * This function returns the integral of Psi_r with respect to Mino time given
 * the Fourier coefficients of Psi_r  (Eq. 3.21)
 */
...
/*!
 * This function returns the integral of Psi_theta with respect to Mino time
 * given the Fourier coefficients of Psi_theta  (Eq. 3.22)
 */
 ...
 */
int korb_specint(double complex* amp, double large, int n, korb_params orbpar,
                 double complex (*func)(double, int, korb_params)) {
  int N = 2;
  double dx = 2 * M_PI / N;
...
/*!
 * Takes a function pointer and finds a set of Fourier coefficients via DFT
 * (including convergence assessment)
 */
int korb_dft(int* N, korb_params orbpar,
             double complex (*func)(double, int, korb_params), double** amps) {
  int num = 2;
  ...
  while (small / large > orbpar.err ||
         cabs(camps[num - 1]) / large > orbpar.err ||
         cabs(camps[num - 2]) / large > orbpar.err || num < 8) {
    num += 1;
 ...
 */
int korb_dct(int num, double period, korb_params orbpar,
             double (*func)(double, korb_params), double amps[]) {
  fftw_plan p;
...
/*!
 * Takes a function pointer and calculates DCT coefficients via FFTW (including
 * convergence assessment)
 */
int korb_getamps(int* num, double period, korb_params orbpar,
                 double (*func)(double, korb_params), double** amps) {
  *num = 4;
  *amps = malloc(*num * sizeof(double));
  ...
  for (i = 0; i < *num; i++)
    (*amps)[i] = 1.0;
  double scale = 1.0;
  while (fabs((*amps)[*num - 2]) / scale > orbpar.err ||
         fabs((*amps)[*num - 1]) / scale > orbpar.err) {
    *num = *num + 2;
...
double korb_rsfromrsubtrplus(double rsubtrplus, double a) {
  double rp = 1 + sqrt(1 - a * a);
  return rsubtrplus + rp +
         (rp * log(.5 * rsubtrplus) +
          (rp - 2) * log(.5 * rsubtrplus + rp - 1)) /
             (rp - 1);
}

/*!
 * Residual between guessing the tortoise coordinate value and the actual value
 * (for root finding)
 */
...
/*!
 * Gives the derivative of the tortoise coordinate with respect to r (for root
 * finding)
 */
...
/*!
 * Gives both the residual between guessing the tortoise coordinate value and
 * the actual value and the derivative of the tortoise coordinate with respect
 * to r (for root finding)
 */
...
/*!
 * Uses various initial guess strategies to efficiently and accurately find
 * r-r_+ from the tortoise coordinate via root finding
 */
double korb_rsubtrplusfromrs(double rs, double a) {
  ...
  double guess;
  if (rs < -2)
    guess = 2 * pow(1 - a * a, 1 / rp - .5) *
            exp((a * a - rp + (rp - 1) * rs) / rp);
  else if (rs < 1000)
...
/*!
 * This function gathers all necessary numerical techniques to determine generic
 * orbital trajectories of a test mass around a Kerr black hole. The general
 * strategy involves spectral calculation of Fourier coefficients describing the
 * rate-of-change of position, then the position itself is determined by
 * integrating the Fourier series term-by-term. All necessary information for
 * reconstructing the position at a certain time is encoded in the "korb_params"
 * data structure. It adaptively handles convergence testing and memory
 * allocation (but a separate function is needed to free the allocated memory).
 */
int korb_getparams(int eccentric, int inclined, double a, double p, double e0,
                   double x0, double err, korb_params* orbpar) {
  double e, x;
  // Checks are performed to ensure consistency between the boolean flags
  // indicating the presence of eccentricity and/or inclination with the
  // floating point values of eccentricity and/or inclination. Other checks are
  // also made to exclude scenarios that are unphysical or beyond the
  // capabilities of this code.
  if (eccentric == 0) {
    if (fabs(e0) > 1.0e-14) {
      printf(
          "\nBecause you made the 'eccentric' argument zero, the orbital "
          "eccentricity will be set to zero (regardless of your e input).\n\n");
    }
    orbpar->eccentric = 0;
    e = 5.0e-16;
  } else {
    if (fabs(e0) < 1.0e-14) {
      printf(
          "\nBecause you input such a small (near zero) orbital eccentricity "
          "value, the 'eccentric' parameter will be set to zero.\n\n");
      orbpar->eccentric = 0;
      e = 5.0e-16;
    } else if (fabs(e0) > 0.9 + 5.0e-16) {
      printf(
          "\nWarning, eccentricities larger than 0.9 not supported, setting e "
          "= 0.9\n\n");
      orbpar->eccentric = 1;
      e = 0.9;
    } else if (e0 < 0) {
      printf(
          "\nWarning, negative eccentricity, the eccentricity will be made "
          "positive.\n\n");
      orbpar->eccentric = 1;
      e = fabs(e0);
    } else {
  ...
  if (inclined == 0) {
    if (fabs(1 - fabs(x0)) > 1.0e-14) {
      printf(
          "\nBecause you made the 'inclined' argument zero, this orbit is "
          "assumed to be equatorial (regardless of your x input). Prograde or "
          "retrograde implied from the sign of your x argument.\n\n");
    }
    if (x0 >= 0)
      x = cos(5.0e-16);
    else
    ...
    if (fabs(1 - fabs(x0)) <= 1.0e-14) {
      printf(
          "\nBecause you input such a small (near zero) inclination angle "
          "value (|x| is near 1), the 'inclined' parameter will be set to "
          "zero. Prograde or retrograde implied from the sign of your x "
          "argument.\n\n");
      if (x0 > 0)
        x = cos(1.0e-15);
      else
      ...
      printf(
          "\nWarning, x > 1 is not physical, resetting x = 1 and setting the "
          "'inclined' parameter to zero.\n\n");
      x = cos(1.0e-15);
      orbpar->inclined = 0;
    } else if (x0 < -1 - 1.0e-14) {
      printf(
      ...
      printf(
          "\nWarning, near polar orbits are not supported. Setting |x| = 0.15, "
          "prograde or retrograde implied from the sign of your x "
          "argument.\n\n");
      if (x0 > 0)
        x = 0.15;
      else
  ...
  double r1 = orbpar->r1;
  double r2 = orbpar->r2;
  // The mathematical relationships governing the constants of motion are
  // hard-coded
  orbpar->E = sqrt(
      fabs((2 * a * a * (1 - e * e) * (1 - e * e) * x * x *
                (p * p - a * a * (1 - e * e) * (1 - x * x)) *
                (-(p * p * p * (-4 + 3 * p + e * e * (4 + p))) +
                 a * a * (-1 + e * e) * (-1 + e * e) * p * p * (-2 + x * x) +
                 a * a * a * a * (-1 + e * e) * (-1 + e * e) * (-1 + e * e) *
                     (-1 + x * x)) -
            2 * a * (-1 + e * e) * (-1 + e * e) * x *
                (p * p + a * a * (1 - e * e) * (-1 + x * x)) *
                sqrt(fabs(p *
                          ...
                          (p * p + a * a * (1 - e * e) * (-1 + x * x)) *
                          (p * p * p * p -
                           2 * a * a * (1 + e * e) * p * p * (-1 + x * x) +
                           a * a * a * a * (-1 + e * e) * (-1 + e * e) *
                               (-1 + x * x) * (-1 + x * x)))) +
            ((-4 * e * e + (-2 + p) * (-2 + p)) * p * p * p +
             a * a * a * a * (-1 + e * e) * (-1 + e * e) * (-1 + x * x) *
                 (-p + (-1 + e * e + p) * x * x) +
             a * a * p * p *
                 (2 * (-2 + p + e * e * (2 + p)) -
                  (-3 + e * e * e * e + 2 * p + 2 * e * e * (1 + p)) * x * x)) *
                (p * p * p * p * (-3 - e * e + p) +
                 a * a * a * a * (-1 + e * e) * (-1 + e * e) * (-1 + x * x) *
                     (-1 + e * e - p + (-1 + e * e + p) * x * x) -
                 2 * a * a * p * p *
           ...
           (4 * a * a * (1 - e * e) * (1 - e * e) * x * x *
            ...
            pow((3 + e * e - p) * p * p * p * p -
                ...
                2))));
  orbpar->Lz =
      (2 * a * (1 + e) * (1 + e) * p * x * x * orbpar->E -
       x * sqrt(fabs((a * a * (1 + e) * (1 + e) + p * (-2 - 2 * e + p)) *
                     (-p * p + a * a * (1 + e) * (1 + e) * (-1 + x * x)) *
                     (a * a * (1 + e) * (1 + e) * (-1 + x * x) *
                          (-1 + orbpar->E * orbpar->E) +
                      p * (-2 - 2 * e + p - p * orbpar->E * orbpar->E))))) /
      ((1 + e) *
       ((2 + 2 * e - p) * p + a * a * (1 + e) * (1 + e) * (-1 + x * x)));
  orbpar->Q =
      (p * ((1 + e) * (1 + e) * orbpar->Lz * orbpar->Lz * (2 + 2 * e - p) -
            4 * a * (1 + e) * (1 + e) * (1 + e) * orbpar->Lz * orbpar->E +
            a * a * (1 + e) * (1 + e) *
                (-p + (2 + 2 * e + p) * orbpar->E * orbpar->E) +
            p * p * (2 + 2 * e + p * (-1 + orbpar->E * orbpar->E)))) /
      (a * a * (1 + e) * (1 + e) * (1 + e) * (1 + e) +
       (1 + e) * (1 + e) * p * (-2 - 2 * e + p));
  ...
  orbpar->zp =
      (orbpar->B + orbpar->Lz * orbpar->Lz + orbpar->Q +
       sqrt(fabs(-4 * orbpar->B * orbpar->Q +
                 pow(orbpar->B + orbpar->Lz * orbpar->Lz + orbpar->Q, 2)))) /
      (2 * orbpar->B);
  orbpar->r3 =
      (2 * a * a - orbpar->B * (r1 + r2) +
       sqrt(fabs(r1 * r2 *
                 (-4 * a * a * a * a * orbpar->B * orbpar->Q +
                  r1 * r2 * pow(-2 * a * a + orbpar->B * (r1 + r2), 2)))) /
           (r1 * r2)) /
      (2 * orbpar->B);
  orbpar->r4 =
      -(-2 * a * a * r1 * r2 + orbpar->B * r1 * r1 * r2 +
        orbpar->B * r1 * r2 * r2 +
        sqrt(fabs(r1 * r2 *
                  (-4 * a * a * a * a * orbpar->B * orbpar->Q +
                   r1 * r2 * pow(-2 * a * a + orbpar->B * (r1 + r2), 2))))) /
  ...
  orbpar->Yphi = 0.0;
  orbpar->Ga = 0.0;
  // When appropriate, Fourier amplitudes governing orbital evolution are
  // calculated adaptively
  if (orbpar->inclined != 0) {
    korb_getamps(&(orbpar->dladchinum), 2 * M_PI, *orbpar, korb_dladchi,
                 &(orbpar->dladchiamps));
    ...
    korb_dft(&(orbpar->dchidlanum), *orbpar, korb_dchidlaint,
             &(orbpar->dchidlaamps));
  }
  if (orbpar->eccentric != 0) {
    korb_getamps(&(orbpar->dladpsinum), 2 * M_PI, *orbpar, korb_dladpsi,
                 &(orbpar->dladpsiamps));
    ...
    korb_dft(&(orbpar->dpsidlanum), *orbpar, korb_dpsidlaint,
             &(orbpar->dpsidlaamps));
  }
  if (orbpar->inclined == 0) {
...
/*!
 * The main function takes arguments: eccentric (boolean), inclined (boolean), a
 * (double), p (double), e (double), x (double), lambdaSteps (integer),
 * lambdaMax (double), and from those arguments calculates and prints the
 * constants of motion, orbital frequencies, and position at equally spaced Mino
 * times. Must compile with flag '-DLINK_DEFAULT' when not linking through
 * another program.
 */
#ifdef LINK_DEFAULT
int main(int argc, char** argv) {
  ...
  int num;
  double lambdamax;
  // While the code is capable of sacrificing accuracy in favor of a bit more
  // efficiency, in this main function the relative error is fixed at a minimal
  // value (near the limitations of double precision)
  double err = 1.0e-15;
  ...
  printf("Omega_th = %.15f\n", orbpar.wth);
  printf("Omega_r = %.15f\n", orbpar.wr);
  // Equipped with the information inside "orbpar", positions are reconstructed
  // and printed at various Mino time values based on the user inputs
  int j;
